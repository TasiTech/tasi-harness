...
export class MemoryStore {
  ...
  inferDomains(intent: string): MemoryDomain[] {
    return inferMemoryDomains(normalizeLineBreaks(intent));
  }
  ...
  renderPromptBlock(query: MemoryQueryOptions = {}): string {
    ...
    return (['memory', 'user'] as const)
      .map((target) => {
        ...
        const domainSummary = [...new Set(entries.map((entry) => normalizeMemoryDomain(entry.domain)))].join(', ') || DEFAULT_DOMAIN;
        const contextBits = [
          resolvedQuery.sessionId ? `session=${resolvedQuery.sessionId}` : '',
          resolvedQuery.domain ? `domain=${normalizeMemoryDomain(resolvedQuery.domain)}` : '',
        ...
        const body = entries.length > 0
          ? entries
              .map((entry) => {
                ...
                return `- [${normalizeMemoryDomain(entry.domain)} | ${scope}] ${entry.content}`;
              })
              .join('\n')
          : '(empty)';
        return [
  ...
  private applyAdd(entries: MemoryEntry[], target: MemoryTarget, content: string, options: MemoryMutationOptions): MemoryEntry[] {
    ...
    const requestedDomain = options.domain ? normalizeMemoryDomain(options.domain) : undefined;
  ...
  private applyClear(entries: MemoryEntry[], request: MemoryClearRequest): MemoryEntry[] {
    ...
    const domain = mode === 'domain' ? normalizeMemoryDomain(request.domain) : undefined;
    ...
    const next = entries.filter((entry) => {
      ...
      if (mode === 'domain' && normalizeMemoryDomain(entry.domain) !== domain) return true;
      return false;
    });
  ...
  private normalizeEntry(input: unknown, index: number): MemoryEntry | null {
    ...
    return {
      ...
      sessionId,
      domain: normalizeMemoryDomain(raw.domain),
      content,
  ...
  private searchEntries(entries: MemoryEntry[], query: MemoryQueryOptions | undefined, defaultLimit: number): MemoryEntry[] {
    ...
    const domain = query?.domain ? normalizeMemoryDomain(query.domain) : undefined;
    ...
    const filtered = entries
      ...
      .filter((entry) => (domain ? normalizeMemoryDomain(entry.domain) === domain : true))
      .map((entry) => ({
        entry,
  ...
  private scoreEntry(
    ...
    const entryDomain = normalizeMemoryDomain(entry.domain);
    if (context.domain) {
  ...
  private domainUsage(entries: MemoryEntry[]): MemoryDomainUsage[] {
    ...
    for (const entry of entries) {
      const domain = normalizeMemoryDomain(entry.domain);
  ...
  private upsertAutoSessionMemory(entries: MemoryEntry[], record: SessionRecord): MemoryEntry[] | null {
    ...
    if (
      ...
      normalizeMemoryDomain(existing.domain) === snapshot.domain &&
  ...
  private findUniqueIndex(entries: MemoryEntry[], target: MemoryTarget, oldText: string, options: MemoryMutationOptions): number {
    ...
    const domain = options.domain ? normalizeMemoryDomain(options.domain) : undefined;
