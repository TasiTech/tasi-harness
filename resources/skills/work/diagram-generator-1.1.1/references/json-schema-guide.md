...
Complete Draw.io-first schema for diagram specification passed to `mcp-diagram-generator` MCP server. For formal方案/设计文档, set `format` to `drawio`; do not create Mermaid output.
...
```json
{
  ...
  "properties": {
    "format": {
      "type": "string",
      "enum": ["drawio"],
      "description": "Target diagram format for formal deliverables"
    },
    "title": {
      "type": "string",
      "description": "Diagram title used as page name in drawio"
    },
    "elements": {
      "type": "array",
...
## Sequence Diagram Example (Draw.io)
...
- Generated Draw.io XML must use canonical object fields: `mxGeometry as="geometry"`, edge endpoint `mxPoint as="sourcePoint"` / `as="targetPoint"`, and waypoint arrays as `<Array as="points">`.
- Never emit `asGeometry="true"` or `asPoint="true"`; diagrams.net may fail with `Could not add object for mxGeometry`.
- Save `.drawio` sources as UTF-8 without BOM and include an XML declaration. BOM-prefixed files can be rejected by diagrams.net with `Invalid file data`.
