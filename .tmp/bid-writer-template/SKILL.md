...
Final deliverables must not contain reference-template markers such as `{变量名}`, `{证据槽：...}`, `file_type: reference_template`, `is_deliverable: false`, `使用边界`, `模板选择`, or `不进入交付物的内容`. These belong only to `references/bid-template.md` and working notes.
...
- If the tender does not provide a complete outline, read [references/bid-template.md](references/bid-template.md) and use it only as a structural module library. Do not copy its reference notes, variables, or evidence-slot markers into the final deliverable.
...
- `references/bid-template.md`: read only when the tender lacks a complete required outline, the user asks for a generic bid template, or Stage 2 needs a fallback structural module library. Treat it as reference-only and exclude its guidance text from deliverables.
